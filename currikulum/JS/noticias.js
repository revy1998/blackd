...
function resaltarNoticiasQueContengan(palabra){
  for(var contador = 0; contador<noticias.length; contador++){
    ...
    if(contienePalabra(noticiaActual,palabra)){
      cambiarColor(noticiaActual, 'rgb(168, 170, 181)');
    }
}
}

function resaltarNoticiasQueContenganla(palabra){
  for(var contador = 0; contador<noticias.length; contador++){
    noticiaActual = noticias[contador];
    if(contienePalabra(noticiaActual,palabra)){
      cambiarColor(noticiaActual, 'rgb(186, 165, 182)');
    }
}
}
...
resaltarNoticiasQueContenganla("argentino")
recortarNoticias(10)
